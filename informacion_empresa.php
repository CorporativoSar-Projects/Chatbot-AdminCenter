...
// Si viene una acción específica, redirigir a la página de edición
if (isset($_GET['accion']) && $_GET['accion'] == 'editar' && $info_existente) {
    header("Location: editar_info_empresa.php");
    exit();
}
?>
<!DOCTYPE html>
...
<body>
            ...
            <?php if ($info_existente): ?>
                <div class="alert alert-warning">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <strong>¡Ya tienes información guardada!</strong>
                            <p class="mb-0 mt-2">¿Qué deseas hacer?</p>
                        </div>
                        <div>
                            <a href="editar_info_empresa.php?accion=editar" class="btn btn-primary me-2" style="width:100%;">
                                <i class="bi bi-pencil-square"></i> Editar Información Existente
                            </a>
                            <!--<button type="button" class="btn btn-outline-secondary" onclick="continuarNuevo()">
                                <i class="bi bi-plus-circle"></i> Crear Nueva Información
                            </button>-->
                        </div>
                    </div>
                </div>
    ...
    <script>
        // Función para mostrar el formulario para nueva información
        function continuarNuevo() {
            if (confirm('¿Está seguro de crear nueva información? Esto sobrescribirá la información existente.')) {
                document.getElementById('nuevoFormulario').style.display = 'block';
                document.querySelector('.alert-warning').style.display = 'none';
            }
        }

        // Si hay información existente, añadir el cierre del div
        <?php if ($info_existente): ?>
            document.addEventListener('DOMContentLoaded', function() {
                // Añadir el cierre del div después del formulario
                const form = document.getElementById('formInfoEmpresa');
                const closeDiv = document.createElement('div');
                closeDiv.id = 'cierreNuevoFormulario';
                closeDiv.style.display = 'none';
                form.parentNode.insertBefore(closeDiv, form.nextSibling);

                // Mover el formulario dentro del div si se hace clic en "Nuevo"
                window.continuarNuevo = function() {
                    if (confirm('¿Está seguro de crear nueva información? Esto sobrescribirá la información existente.')) {
                        document.getElementById('nuevoFormulario').style.display = 'block';
                        document.querySelector('.alert-warning').style.display = 'none';
                        document.getElementById('cierreNuevoFormulario').style.display = 'block';
                        document.getElementById('cierreNuevoFormulario').innerHTML = '</div>';
                    }
                }
            });
        <?php endif; ?>
