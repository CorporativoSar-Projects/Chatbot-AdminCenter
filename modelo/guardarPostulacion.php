...
$stmt = $conexion->prepare("SELECT id_candidate, nombre_candidate, apellidop_candidate, apellidom_candidate, tel_candidate, CV_candidate, CV_id_onedrive FROM candidato WHERE correo_candidate = ?");
$stmt->bind_param("s", $correo);
$stmt->execute();
$stmt->store_result();
$stmt->bind_result($id_candidate, $nombreExist, $apellidopExist, $apellidomExist, $telExist, $cvAntiguoDb, $cvAntiguoIdDb);
...
if ($existe) {
    ...
    $cvAntiguoId = $cvAntiguoIdDb; // Obtenemos ID real del CV en OneDrive
}
...
if (isset($_FILES['CV_candidate']) && $_FILES['CV_candidate']['error'] === 0) {
    ...
    if ($ext !== 'pdf') { echo json_encode(["error"=>"Solo se permiten archivos PDF"]); exit; }
    ...
    if (!$linkOneDriveData) { echo json_encode(["error"=>"No se pudo subir el CV a OneDrive"]); exit; }
...
if ($existe) {
    ...
    if ($nombre !== "") { $updates[]="nombre_candidate=?"; $params[]=$nombre; $types.="s"; } else { $nombre=$nombreExist; }
    if ($apellidop !== "") { $updates[]="apellidop_candidate=?"; $params[]=$apellidop; $types.="s"; } else { $apellidop=$apellidopExist; }
    if ($apellidom !== "") { $updates[]="apellidom_candidate=?"; $params[]=$apellidom; $types.="s"; } else { $apellidom=$apellidomExist; }
    if ($telefono !== "") { $updates[]="tel_candidate=?"; $params[]=$telefono; $types.="s"; } else { $telefono=$telExist; }
    if ($nuevoCV !== $cvAntiguo) { 
        $updates[]="CV_candidate=?"; $params[]=$nuevoCV; $types.="s"; 
        $updates[]="CV_id_onedrive=?"; $params[]=$cvNuevoId; $types.="s";
    }

    if (count($updates)>0) {
        $sql="UPDATE candidato SET ".implode(", ", $updates)." WHERE id_candidate=?";
        $params[]=$id_candidate; $types.="i";
    ...
    if (!$nuevoCV) { echo json_encode(["error"=>"Debes subir un CV para registrarte."]); exit; }
...
");
    ...
    if (!file_exists($jsonPath)) { echo json_encode(["error"=>"No se encontró el archivo JSON para la empresa $id_emp"]); exit; }
    ...
    if (!empty($configData['conversacion']) && is_array($configData['conversacion'])) {
        foreach($configData['conversacion'] as $conv){
            if(!empty($conv['urlInforme'])) { $urlVacantesCSV = $conv['urlInforme']; break; }
        }
    }
    if(!$urlVacantesCSV){ echo json_encode(["error"=>"No se encontró la URL del CSV de vacantes en el JSON"]); exit; }
    ...
    curl_close($ch);
    if($csvContent===false){ echo json_encode(["error"=>"No se pudo leer el CSV: $curlErr"]); exit; }

    $tmp=fopen('php://memory','r+'); fwrite($tmp,$csvContent); rewind($tmp);
    fgetcsv($tmp); // encabezado
    while(($data=fgetcsv($tmp))!==false){ $vacantesMap[trim($data[0])] = trim($data[2]); }
    fclose($tmp);
    ...
    if(!$rutaDestino){ echo json_encode(["error"=>"No se ha configurado la ruta de destino para esta empresa"]); exit; }
    ...
    function decryptSFTP($string){ return openssl_decrypt($string,"AES-256-CBC",SECRET_KEY,0,SECRET_IV); }
    ...
    if(!empty($rowRuta['servidor'])){
        ...
        } else { error_log("No se pudo autenticar SFTP empresa $id_emp"); }
    }

    $cvLinkTexto = $nuevoCV ? '<a href="'.$nuevoCV.'" target="_blank">Ver CV</a>' : 'No disponible';

  // --- CSV interno global ---
    $csvInterno=__DIR__."/../interno/exports/postulaciones_internas.csv";
    ...
    $stmtInterno->execute(); $resInterno=$stmtInterno->get_result();
